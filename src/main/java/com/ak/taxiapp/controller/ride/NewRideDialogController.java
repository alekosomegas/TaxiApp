...
public class NewRideDialogController extends Controller {
    // ------------------------------------------------------------------ //
    //region// ---------------------------- VARIABLES --------------------------- //
    ...
    public void insert() throws SQLException {
        ...
        try {
            RideDAO.insert(dtDate.getValue().toString(),
                    getRidesTimes(spStartH, spStartM),
                    getRidesTimes(spEndH, spEndM),
...
//            resultArea.setText("Problem occurred while inserting client " + e);
    ...
    @NotNull
    private String getRidesTimes(Spinner h, Spinner m) {
        String hValue = h.getValue().toString();
        String mValue = m.getValue().toString();
        if(hValue.length() == 1) {hValue = "0"+hValue;}
        if(mValue.length() == 1) {mValue = "0"+mValue;}
...
//        }
    ...
    private String getStops() {
        tfStops = "";
        int size = stopsTfList.size();
        for (TextField tf : stopsTfList) {
            tfStops = tfStops + tf.getText();
            if (stopsTfList.indexOf(tf) != size-1) {
                tfStops = tfStops + " ; ";
            }
        }
